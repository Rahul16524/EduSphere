\# Sprint 0 - Project Initialization



\## Sprint Goal



Prepare the repository and development environment before writing any application code.



\---



\## Story 1



\*\*Title:\*\* Initialize Repository Structure



\*\*Assigned To:\*\* Tech Lead



\*\*Priority:\*\* High



\*\*Status:\*\* In Progress



\### Description



Create the standard repository structure for the EduSphere project.



\### Acceptance Criteria



\- \[x] GitHub repository created

\- \[x] Repository cloned for all team roles

\- \[ ] Repository folder structure created

\- \[ ] Initial README created

\- \[ ] Initial .gitignore created

\- \[ ] Initial LICENSE created

\- \[ ] Repository pushed to GitHub : Done



\---



\## Next Story



\*\*Title:\*\* Setup Eureka Discovery Server



\*\*Assigned To:\*\* Developer 1



\*\*Status:\*\* Not Started



\### Acceptance Criteria



\- Spring Boot project created

\- Application starts successfully

\- Eureka dashboard opens

\- Code committed

\- Code reviewed





==================

\## Story 2



\*\*Title:\*\* Setup Eureka Discovery Server



\*\*Assigned To:\*\* Developer 1



\*\*Priority:\*\* Highest



\*\*Status:\*\* Completed



\### Description



Create and configure the Eureka Discovery Server for the EduSphere microservices architecture.



\### Acceptance Criteria



\- \[x] Spring Boot project created

\- \[x] Application starts successfully

\- \[x] Eureka dashboard opens

\- \[x] Discovery Server configured

\- \[x] Manual testing completed

\- \[ ] Code reviewed

\- \[ ] Merged into main



======================

\## Story 3



\*\*Title:\*\* Setup Config Server



\*\*Assigned To:\*\* Developer 2



\*\*Priority:\*\* Highest



\*\*Status:\*\* In Progress



\### Acceptance Criteria



\- \[ ] Spring Boot project created

\- \[ ] Config Server configured

\- \[ ] Git repository configured

\- \[ ] Configuration loaded successfully

\- \[ ] Manual testing completed

\- \[ ] Code reviewed

\- \[ ] Merged into main

